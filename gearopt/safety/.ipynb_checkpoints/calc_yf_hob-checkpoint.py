...
def calc_yf_hob(d, d_Na, z, x, eps_alpha, m_n, alpha_n, h_fPf, rho_fPf, beta, eps_beta):
    """
    Gradient-friendly root geometry factors for a hobbed gear tooth.
    Translated from MATLAB calcYF_hob.m (1:1 functional equivalence).

    Returns
    -------
    Y_F : float
        Root form factor (dimensionless)
    Y_S : float
        Stress correction factor (dimensionless)
    """
    tiny = 1e-12
    ...
    # Iterative theta solver (damped Newton with softclip)
    ...
    for _ in range(12):
        ...
        theta_clipped = softclip(theta_new, 5 * np.pi / 180, 85 * np.pi / 180, 20)
    ...
    rho_F = rho_fP + m_n * safe_div(
        2 * G**2 / np.cos(theta), z_n * np.cos(theta) ** 2 - 2 * G
    )
    h_Fe = (
        m_n
        / 2
        * (
            (np.cos(gamma_e) - np.sin(gamma_e) * np.tan(alpha_Fen)) * safe_div(d_en, m_n)
